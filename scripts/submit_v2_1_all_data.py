...
def inference(rank, world_size, temp_dir, args, runs, particle_names):
    ...
    n_experiments = len(args.experiment_list)
    cfg_list = load_configs(args.experiment_list)

    preprocess_versions = list(set(cfg.data.preprocess_version for cfg in cfg_list))

    if len(args.inference_overlap) == 1:
        inference_overlap = args.inference_overlap * 3
    elif len(args.inference_overlap) == 3:
        inference_overlap = args.inference_overlap
    else:
        raise ValueError("inference_overlap must have length 1 or 3.")

    if len(args.discard_ratio) == 1:
        discard_ratio = args.discard_ratio * 3
    elif len(args.discard_ratio) == 3:
        discard_ratio = args.discard_ratio
    else:
        raise ValueError("discard_ratio must have length 1 or 3.")
    ...
    for i_run, run in enumerate(tqdm(runs)):
        ...
        for p, name in enumerate(particle_names):
            ...
            xyz_pred, cc = get_centroids_from_pred(
                preds_fold_mean[p], args.thresh[p], voxel_spacing=args.voxel_size, return_cc=True
            )

            if name == "ribosome" and args.thresh_ribosome_low > 0:
                xyz_predict_low, cc_low = get_centroids_from_pred(
                    preds_fold_mean[p],
                    args.thresh_ribosome_low,
                    voxel_spacing=args.voxel_size,
                    return_cc=True,
                )

                additional_xyz = []
                for i in range(1, cc_low.max()):
                    cc_low_ = cc_low == i
                    cc_high_in_low = cc[cc_low_]
                    ...
                    if len(cc_high_in_low_not_zero) > 1:
                        additional_xyz.append(xyz_pred[cc_high_in_low_not_zero - 1].mean(0))

                if len(additional_xyz) > 0:
                    additional_xyz = np.array(additional_xyz)
                    xyz_pred = np.concatenate([xyz_pred, additional_xyz])
                else:
                    xyz_pred = xyz_pred
...
def main():
    ...
    parser.add_argument("--thresh_ribosome_low", type=float, default=0.0)
    parser.add_argument(
        "--fp16_mode_list",
        type=str2bool,
        nargs="+",
    ...
    parser.add_argument("--inference_overlap", type=float, nargs="+", default=[0.2, 0.2, 0.2])
    parser.add_argument("--discard_ratio", type=float, nargs="+", default=[0.0, 0.0, 0.0])
