...
# 関連する関数を定義
def get_ckpt_name(ckpt_type, fold, epoch=None):
    if ckpt_type == "last":
        if fold == 0:
            ckpt_name = "last.ckpt"
        else:
        ...
        assert epoch is not None, "epoch must be specified when ckpt_type is 'epoch'"
        ckpt_name = f"fold{fold}_epoch_{epoch:03d}.ckpt"
    else:
...
def load_tomogram(run, voxel_size, tomo_type, preprocess_version):
...
def inference(rank, world_size, temp_dir, args, runs, particle_names):
    ...
    for i_run, run in enumerate(tqdm(runs)):
        ...
        for preprocess_version in preprocess_versions:
            data_preprocess_dict[preprocess_version] = load_tomogram(
                run, args.voxel_size, args.tomo_type, preprocess_version
            )
        ...
        for experiment, compiled_model_dir, cfg, fp16_mode, batch_size_pred, n_fold in zip(
            ...
            for fold in range(n_fold):
                ...
                save_path = os.path.join(
                    compiled_model_dir,
                    f"trt_{experiment}_fold{fold}_{args.ckpt_type}_bs{batch_size_pred}_fp16{fp16_mode}.ep",
                )
...
def main():
    ...
    parser.add_argument("--voxel_size", type=int, default=10)
    ...
    parser.add_argument("--n_fold_list", type=int, nargs="+", default=[5])
