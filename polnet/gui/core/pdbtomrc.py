...
def add_cloud_gauss(tomo, coords, g_std, value):
    ...
    del Y
    del Z
    gauss = (1 / (g_std * g_std * g_std * math.sqrt(2 * np.pi))) * np.exp(-R / (2.0 * g_std * g_std))
...
def pdb_2_mrc(file_name, output_file, apix, res, offset, het, selected_atoms, model):
    ...
    # Initialize the atomdefs dictionary
    atomdefs = {}

    # Populate atomdefs with atomic number and atomic mass for each element
    for element in periodictable.elements:
        if element.name != "n/a":  # Exclude non-applicable entries
            atom_symbol = element.symbol.upper()
            atomdefs[atom_symbol] = (element.number, element.mass)

    atomdefs["HH"] = (1.0, 1.00794)  # HH is a hydrogen attached to a hydrogen
    atomdefs["HD"] = (1.0, 1.00794)  # HD is a hydrogen attached to a deuterium
...
def pdbx_2_mrc(file_name, output_file, apix, res, offset, het, selected_atoms, model):
    """
    Converts a PDBx/mmCIF file to an MRC file.

    :param file_name: Input PDBx/mmCIF file name.
    :param output_file: Output MRC file name.
    :param apix: Voxel size per Ångström, scale of the tomogram resolution.
    :param res: Tomogram resolution in Ångströms.
    :param offset: Additional offset for the tomogram size.
    :param het: Flag to include HETATM atoms (True) or not (False).
    :param selected_atoms: List of protein chains to include in the tomogram (None to include all).
    :param model: Model number in the structure to use (None to include all models).
    """
    print("Processing PDBx/mmCIF:", file_name)

    # Parse the mmCIF file
    parser = MMCIFParser(QUIET=True)
    structure = parser.get_structure("structure_id", file_name)

    # Initialize atomdefs dictionary
    atomdefs = {}
    for element in periodictable.elements:
        if element.name != "n/a":
            atomdefs[element.symbol.upper()] = (element.number, element.mass)
    ...
    for model_struct in structure:
        if model is not None and model_struct.id != model:
            continue  # Skip other models

        for chain in model_struct:
            if selected_atoms and chain.id not in selected_atoms:
                continue  # Skip non-selected chains

            for residue in chain:
                for atom in residue:
                    if not het and atom.is_disordered():
                        continue  # Skip HETATM if not requested
    ...
    # Calculate center of mass and box size
    cm = np.mean(coords, axis=0)
    print("\tCenter of mass:", cm)
    axis_size = np.ceil((amax - amin + 2 * offset)).astype(np.int32)
    max_dimension = np.max(axis_size)
