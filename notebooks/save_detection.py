...
from src.czii_utils.inference import sliding_window_inference as my_sliding_window_inference
...
setting_name = "250101_pre_env2b2-250102_pre_unet-250103_env2b2"
...
for fold in range(n_fold):
    ...
    for experiment in experiment_list:
        ...
        if pred_size is not None:
            ...
            _pred_size = cfg.data.volume_size
...
# %%
del preprocessed_data, data, image
gc.collect()

# %% save results
save_dir = f"/workspace/logs/detection_results/{setting_name}"
os.makedirs(save_dir, exist_ok=True)

for fold in range(n_fold):
    save_dir_fold = f"{save_dir}/fold{fold}"
    os.makedirs(save_dir_fold, exist_ok=True)

    preds = all_preds[fold].numpy().astype(np.float16)
    np.save(f"{save_dir_fold}/preds_detection.npy", preds)

    for particle_idx, particle_name in enumerate(particle_names):
        best_thresh = best_thresholds[particle_idx]
        hit_pred_list = all_hit_pred_lists[fold][best_thresh][particle_name]
        miss_truth_list = all_miss_truth_lists[fold][best_thresh][particle_name]
        fp_pred_list = all_fp_pred_lists[fold][best_thresh][particle_name]
        cc_list = all_cc_lists[fold][best_thresh][particle_name]
        np.save(f"{save_dir_fold}/hit_pred_list_{particle_name}.npy", hit_pred_list)
        np.save(f"{save_dir_fold}/miss_truth_list_{particle_name}.npy", miss_truth_list)
        np.save(f"{save_dir_fold}/fp_pred_list_{particle_name}.npy", fp_pred_list)
        np.save(f"{save_dir_fold}/cc_list_{particle_name}.npy", cc_list)

np.save(f"{save_dir}/best_thresholds.npy", best_thresholds)
np.save(f"{save_dir}/best_f_beta_list.npy", best_f_beta_list)
