...
class CZIILitModule(LightningModule):
    ...
    def __init__(
        ...
        criterion_focal: torch.nn.Module,
        ...
        scale_factor_focalloss: float = 10,
        focalloss_start_epoch: int = 10,
        loss_base_final_ratio: float = 0.5,
    ) -> None:
        ...
        self.criterion_focal = criterion_focal
    ...
    def model_step(
        ...
        loss_base = self.criterion_classmap(outputs["classmap"], batch["maps"])
        loss_focal = self.hparams.scale_factor_focalloss * self.criterion_focal(
            outputs["classmap"], batch["maps"]
        )
        # Adjust the weights of loss_base and loss_focal based on the specified final ratio
        if self.current_epoch < self.hparams.focalloss_start_epoch:
            loss = loss_base
        else:
            total_epochs = self.trainer.max_epochs - self.hparams.focalloss_start_epoch
            elapsed_epochs = self.current_epoch - self.hparams.focalloss_start_epoch
            loss_base_ratio = 1 - (1 - self.hparams.loss_base_final_ratio) * (elapsed_epochs / total_epochs)
            loss_focal_ratio = 1 - loss_base_ratio
            loss = loss_base_ratio * loss_base + loss_focal_ratio * loss_focal
