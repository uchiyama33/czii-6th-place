...
def encode_for_resnet(e, x, B, depth_scaling, num_stages=4):
    encode = []
    x = e.conv1(x)
    x = e.bn1(x)
    x = e.act1(x)
...
def encode_for_convnext(e, x, B, depth_scaling, num_stages=4):
    encode = []
...
def encode_for_efficientnet(e, x, B, depth_scaling, feature_blocks, num_stages=4):
    encode = []
    x = e.conv_stem(x)
    x = e.bn1(x)
    ...
    def __init__(
        ...
        if "convnext" in model_name:
            ...
            self.encode_func = partial(
                encode_for_convnext, depth_scaling=depth_scaling, num_stages=num_stages
            )
            ...
            depth_scaling = [2, 2, 2, 2, 1][: num_stages + 1]
            spatial_scaling = [2, 2, 2, 2, 2][: num_stages + 1]
            self.encode_func = partial(encode_for_resnet, depth_scaling=depth_scaling, num_stages=num_stages)
        else:
