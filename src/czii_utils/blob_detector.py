# https://www.kaggle.com/code/kharrington/blobdetector/notebook
...
DEVICE = "cuda"
OUTPUT_CSV_PATH = "/workspace/tmp/submission.csv"
tomo_type = "denoised"
...
def erode_dilate_mask(mask, radius):
    ...
    struct_elem_tensor = (
        torch.tensor(struct_elem, dtype=torch.float32, device=DEVICE).unsqueeze(0).unsqueeze(0)
    )
    ...
    for run in tqdm(root.runs):
        ...
        for obj in root.pickable_objects:
            ...
            if centroids:
                ...
                # Store pick set
                # pick_set.points = [
                #     {"x": c[2] * voxel_spacing, "y": c[1] * voxel_spacing, "z": c[0] * voxel_spacing}
                #     for c in centroids
                # ]
                # pick_set.store()
...
if __name__ == "__main__":
    ...
    results = process_all_runs(root=root, session_id="0", user_id="blobDetector", voxel_spacing=10)
